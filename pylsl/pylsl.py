...
class StreamInlet:
    ...
    def pull_chunk(self, timeout=0.0, max_samples=1024):
        ...
        samples = [
            [buffer[0][s * num_channels + c] for c in range(num_channels)]
...
def resolve_stream(*args, timeout=10):
    if len(args) == 0:
        ...
        if len(args) == 1:
            ...
            if len(args) == 2:
                return resolve_byprop(args[0], args[1], timeout=timeout)
            else:
                return resolve_byprop(args[0], args[1], args[2], timeout=timeout)
...
if not libpath:
    raise RuntimeError(
        "library " + libname + " was not found - make sure "
